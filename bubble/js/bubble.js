		...
		engine: function () {
			...
			var bubbleconstructor = function (spec) {
				...
				that.onmousedown = function () {
					...
					if (YATE.games.bubble.engine.floodcheck({x: this.x, y: this.y, type: this.type})) {
						gamevars.points[gamevars.cycle] = 0;
						YATE.games.bubble.engine.floodrem({x: this.x, y: this.y});
						YATE.games.bubble.engine.setscore();
						gamevars.cycle += 1;
					}
					return false;
				};
			...
			return {
				...
				start: function () {
					gamevars.gamesplayed = parseInt(YATE.cookie.get('bubble_gamesplayed'), 10) || 0;
					gamevars.yourbest = parseInt(YATE.cookie.get('bubble_bestscore'), 10) || 0;
					YATE.games.bubble.engine.cleanup();
					YATE.games.bubble.engine.newlevel();
				},
				newlevel: function () {
					if (gamevars.level === 5) {
						gamevars.colors = 4;
					...
					YATE.games.bubble.engine.setlevel();
					...
					YATE.games.bubble.engine.bulkshow(newbubbles);
					
					//if the level has ended when we just filled up the area, it's over
					if (YATE.games.bubble.engine.levelendcheck()) {
						setTimeout(YATE.games.bubble.engine.gameover, 600);
					}
				},
				setscore: function () {
				...
				floodcheck: function (coords) {
					...
					if (rv === 0) {
						...
						if (coords.tried) {
							...
							dir.tried = true;
							return YATE.games.bubble.engine.floodcheck(dir);
						}
					}
				},
				...
				floodrem: function (coords) {
					...
					step(coords);
					YATE.games.bubble.engine.fall();

					//if the level ends after a click, we start a new one
					if (YATE.games.bubble.engine.levelendcheck()) {
						setTimeout(YATE.games.bubble.engine.newlevel, 200);
					}
				},
				fall: function () {
					...
					YATE.games.bubble.engine.bulkfall(bubbles);
				},
				levelendcheck: function () {
					var i;
					var bubbles = document.getElementById('bt_realarea').getElementsByTagName('span');
					for (i = 0; i < bubbles.length; i += 1) {
						if (YATE.games.bubble.engine.floodcheck({x: bubbles[i].x, y: bubbles[i].y, type: bubbles[i].type})) {
							return false;
						}
					}
					return true;
				...
				gameover: function () {
					...
					YATE.cookie.set('bubble_gamesplayed', gamevars.gamesplayed);
					if (gamevars.score > gamevars.yourbest) {
						YATE.say.alert('<h1>Game over</h1>Hey, you\'ve beaten your highscore!');
						gamevars.yourbest = gamevars.score;
						YATE.cookie.set('bubble_bestscore', gamevars.yourbest);
					} else {
						YATE.say.alert('<h1>Game over</h1>How about a new game?');
					}
				}
			};			
		}()
